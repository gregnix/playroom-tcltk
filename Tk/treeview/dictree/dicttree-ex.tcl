...
namespace eval dictree {        ;# "private" functions
  ...
  proc dictdir { dir } {
    ...
    foreach fname [lsort [glob -directory $dir -nocomplain -types f "*"]] {
      ...
      #        # sorted:
      #        foreach item [lsort [array names fstat]] {
      dict set d [file tail $fname] $fname
      #        }
      # faster but unsorted:
...
#dictree::main {*}$argv
#    dictree::main /erweitert/sicher/greg
#    puts "q [ .t selection ]"
#    puts "w [.t configure]"
#}
...
proc listns {{parentns ::}} {
  ...
  # Füge die Variablen des aktuellen Namespaces hinzu
...
# Prozedur, die alle Variablen eines Namespaces auflistet
proc listnsvars {ns} {
    set result [list]
