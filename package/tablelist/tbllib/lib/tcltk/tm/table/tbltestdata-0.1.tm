...
proc generateMixedContentList {numEntries numColumns} {
    ...
    # Bestimme den Datentyp für jede Spalte
    for {set j 1} {$j <= $numColumns} {incr j} {
        lappend columnTypes [expr {int(rand() * 3)}]
    }

    # Generiere die Daten basierend auf den festgelegten Spaltentypen
    for {set i 1} {$i <= $numEntries} {incr i} {
        set entry [list]
        for {set j 1} {$j <= $numColumns} {incr j} {
            set randomType [lindex $columnTypes [expr {$j - 1}]]
            switch $randomType {
                0 {lappend entry [expr {int(rand() * 100)}]}   ;# Zahl
                1 {
...
proc generateRandomNumberList {numEntries numColumns} {
    ...
    for {set i 1} {$i <= $numEntries} {incr i} {
        set entry [list]
        for {set j 1} {$j <= $numColumns} {incr j} {
            lappend entry [expr {int(rand() * 100)}]  ;# Zufällige Zahl zwischen 0 und 100
        }
        lappend largeList $entry
    }
    return $largeList
...
proc generateFixedMixedContentList {numEntries numColumns} {
    ...
    # Definiere die festen Datentypen für die Spalten
    set columnTypes {0 1 2}  ;# 0 = Zahl, 1 = Buchstabe, 2 = Datum
...
proc generateReferenceList {numEntries numColumns} {
    ...
    # Definiere die festen Referenzwerte für die Spalten
    set referenceData {
    ...
    # Fülle die Liste mit den Referenzwerten auf
    for {set i 0} {$i < $numEntries} {incr i} {
        set entry [list]
