    ...
    foreach item $data {
        ...
        for {set i 0} {$i < $length } {incr i} {
            ...
            if {![catch {getAttrValue tree $plist}]} {
                set pid [dict get [dict get [lrange [getAttrValue tree $plist] 0 7]] pid]
                set pcounter [dict get [dict get [lrange [getAttrValue tree $plist] 0 7]] counter]
                set ppid [dict get [dict get [lrange [getAttrValue tree $plist] 0 7]] ppid]
            } else {
...
proc treeAttrToTblLoad {tree colslist } {
    ...
    set liste [concat {*}[walkTree tree {} cmdListAttr]]
    set liste [walkTree tree {} cmdListNode]
    set datalist [list [list root 0 {*}[lrepeat $repeat {}]]]
    set parentlist {-1}
    foreach item $liste {
        set k [lindex $item 0]
        set value [dict get [lrange $item 1 end] value]
